...
func TestCopyAndDeepCody(t *testing.T) {
	...
	// value is same and inner pointer too
	{
		var s = "gz"
		var a = mp{Name: "jd", P: &s}
		ap := &a
	...
	// value is same, but pointer isn't
	{
		...
		r := DeepCopy(ap)
		Assert(t, *r != a, true)
		Assert(t, r.Name, a.Name)
		Assert(t, *r.P, *a.P)
		Assert(t, r.P != a.P, true)
		Assert(t, unsafe.Pointer(r) != unsafe.Pointer(ap), true)
...
CopyFloat32s:
	...
	cpyF := DeepCopy[[]float32](Float32s)
...
CopyInterfaces:
	...
	cpyIf := DeepCopy[[]interface{}](Interfaces)
...
sliceMap:
	...
	cpyM := DeepCopy[[]map[int]string](slMap)
...
func TestIssue9(t *testing.T) {
	...
	copyA := DeepCopy[map[string]*int](testA)
	...
	copyC := DeepCopy[map[*Foo][]string](testC)
	...
	copyD := DeepCopy[map[Bizz]string](testD)
...
func (i *CI) DeepCopy() interface{} {
	return &CI{A: "custom copy"}
}
