...
// copy from https://github.com/sony/gobreaker, but with generic on Execute
...
func NewTwoStepCircuitBreaker[P, R any](st Settings) *TwoStepCircuitBreaker[P, R] {
	return &TwoStepCircuitBreaker[P, R]{
		cb: NewCircuitBreaker[P, R](st),
	}
}
...
func (cb *CircuitBreaker[P, R]) beforeRequest() (uint64, error) {
	cb.mutex.Lock()
	defer cb.mutex.Unlock()
...
func (cb *CircuitBreaker[P, R]) afterRequest(before uint64, success bool) {
	cb.mutex.Lock()
	defer cb.mutex.Unlock()
...
func (cb *CircuitBreaker[P, R]) onSuccess(state State, now time.Time) {
	switch state {
	case StateClosed:
		cb.counts.onSuccess()
	case StateHalfOpen:
...
func (cb *CircuitBreaker[P, R]) onFailure(state State, now time.Time) {
	switch state {
	case StateClosed:
		cb.counts.onFailure()
...
func (cb *CircuitBreaker[P, R]) currentState(now time.Time) (State, uint64) {
	switch cb.state {
	case StateClosed:
...
func (cb *CircuitBreaker[P, R]) setState(state State, now time.Time) {
	if cb.state == state {
		return
	}
...
func (cb *CircuitBreaker[P, R]) toNewGeneration(now time.Time) {
	cb.generation++
	cb.counts.clear()
