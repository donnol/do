...
var defaultCB *CircuitBreaker[any, any]
var customCB *CircuitBreaker[any, any]
var negativeDurationCB *CircuitBreaker[any, any]
...
func pseudoSleep(cb *CircuitBreaker[any, any], period time.Duration) {
	if !cb.expiry.IsZero() {
		cb.expiry = cb.expiry.Add(-period)
	}
}

func succeed(cb *CircuitBreaker[any, any]) error {
	_, err := cb.Execute(nil, func(any) (interface{}, error) { return nil, nil })
	return err
}

func succeedLater(cb *CircuitBreaker[any, any], delay time.Duration) <-chan error {
	ch := make(chan error)
	go func() {
		_, err := cb.Execute(nil, func(any) (interface{}, error) {
			time.Sleep(delay)
			return nil, nil
		})
		ch <- err
...
func succeed2Step(cb *TwoStepCircuitBreaker[any, any]) error {
	done, err := cb.Allow()
	if err != nil {
		return err
	}
...
func fail2Step(cb *TwoStepCircuitBreaker[any, any]) error {
	done, err := cb.Allow()
	if err != nil {
		return err
	}
...
func causePanic(cb *CircuitBreaker[any, any]) error {
	_, err := cb.Execute(nil, func(any) (interface{}, error) { panic("oops") })
	return err
}

func newCustom() *CircuitBreaker[any, any] {
	...
	return NewCircuitBreaker[any, any](customSt)
}

func newNegativeDurationCB() *CircuitBreaker[any, any] {
	var negativeSt Settings
	negativeSt.Name = "ncb"
	negativeSt.Interval = time.Duration(-30) * time.Second
	negativeSt.Timeout = time.Duration(-90) * time.Second

	return NewCircuitBreaker[any, any](negativeSt)
}

func init() {
	defaultCB = NewCircuitBreaker[any, any](Settings{})
	customCB = newCustom()
...
func TestTwoStepCircuitBreaker(t *testing.T) {
	tscb := NewTwoStepCircuitBreaker[any, any](Settings{Name: "tscb"})
...
func TestCustomIsSuccessful(t *testing.T) {
	...
	cb := NewCircuitBreaker[any, any](Settings{IsSuccessful: isSuccessful})
